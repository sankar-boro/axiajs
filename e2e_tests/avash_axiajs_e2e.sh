...
elapsed=0
sleep 2
for port in $axia_ports
do
    echo waiting for node $axia_ip:$port to finish bootstrapping
    is_bootstrapped $axia_ip $port
...
# execute tests
export AVALANCHEGO_IP=$axia_ip
export AVALANCHEGO_PORT=$(echo $axia_ports | cut -d" " -f1)
echo testing on axia $AVALANCHEGO_IP:$AVALANCHEGO_PORT
cd $axiajs_location
