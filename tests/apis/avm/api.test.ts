...
import { CENTIAXC } from "src/utils"
import { MILLIAXC } from "src/utils"
...
describe("AVMAPI", (): void => {
  ...
  test("createFixedCapAsset", async (): Promise<void> => {
    const kp: KeyPair = new KeyPair(axia.getHRP(), alias)
    kp.importKey(
      Buffer.from(
  ...
  describe("Transactions", (): void => {
    ...
    test("buildCreateNFTMintTx", async (): Promise<void> => {
      ...
      const payload: Buffer = Buffer.from("Axia")
