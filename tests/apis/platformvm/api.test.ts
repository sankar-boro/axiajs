...
describe("PlatformVMAPI", (): void => {
  ...
  test("exportAXC", async (): Promise<void> => {
    ...
    const result: Promise<string | ErrorResponseObject> = api.exportAXC(
      username,
      password,
      amount,
      to
  ...
  test("importAXC", async (): Promise<void> => {
    const to: string = "abcdef"
    const username: string = "Robert"
    const password = "Paulson"
    const txID = "valid"
    const result: Promise<string | ErrorResponseObject> = api.importAXC(
      username,
      password,
      to,
      blockchainID
  ...
  describe("Transactions", (): void => {
    ...
    beforeEach(async (): Promise<void> => {
      ...
      keymgr2 = new KeyChain(axia.getHRP(), alias)
      keymgr3 = new KeyChain(axia.getHRP(), alias)
      addrs1 = []
      addrs2 = []
      addrs3 = []
      utxos = []
      ...
      const amount: BN = ONEAXC.mul(new BN(amnt))
    ...
    test("signTx", async (): Promise<void> => {
      const assetID: Buffer = await platformvm.getAXCAssetID()
    ...
    test("buildImportTx", async (): Promise<void> => {
      ...
      const txu2: UnsignedTx = set.buildImportTx(
        ...
        platformvm.getTxFee(),
        await platformvm.getAXCAssetID(),
    ...
    */
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 1", async (): Promise<void> => {
      ...
      const assetID = await platformvm.getAXCAssetID()
      const assetID2 = await platformvm.getAXCAssetID()
    ...
    test("buildAddValidatorTx sort StakeableLockOuts 2", async (): Promise<void> => {
      ...
      const outputidx1: number = 0
      const assetID: Buffer = await platformvm.getAXCAssetID()
      const assetID2: Buffer = await platformvm.getAXCAssetID()
      const utxo1: UTXO = new UTXO(
        codecID,
        txid,
        outputidx0,
    ...
    test("buildAddValidatorTx 2", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONEAXC.mul(new BN(25))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONEAXC.mul(new BN(25)), ONEAXC.mul(new BN(25)))
    ...
    test("buildAddValidatorTx 3", async (): Promise<void> => {
      ...
      const addrbuff2 = addrs2.map((a) => platformvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => platformvm.parseAddress(a))
      const amount: BN = ONEAXC.mul(new BN(3))

      const locktime: BN = new BN(54321)
      const threshold: number = 2

      platformvm.setMinStake(ONEAXC.mul(new BN(3)), ONEAXC.mul(new BN(3)))
