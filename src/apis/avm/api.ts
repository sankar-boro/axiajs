...
import { Defaults, PrimaryAssetAlias, ONEAXC } from "../../utils/constants"
...
export class AVMAPI extends JRPCAPI {
  ...
  protected AXCAssetID: Buffer = undefined
  ...
  refreshBlockchainID = (blockchainID: string = undefined): boolean => {
    ...
    if (
      ...
      this.blockchainID = Defaults.network[`${netid}`].X.blockchainID //default to SwapChain
      return true
    }
   ...
   */
  getAXCAssetID = async (refresh: boolean = false): Promise<Buffer> => {
    if (typeof this.AXCAssetID === "undefined" || refresh) {
      const asset: GetAXCAssetIDParams = await this.getAssetDescription(
        PrimaryAssetAlias
      )
   ...
   * @returns The the provided string representing the AXC AssetID
   */
  setAXCAssetID = (axcAssetID: string | Buffer) => {
  ...
  checkGooseEgg = async (
    ...
    const fee: BN = utx.getBurn(axcAssetID)
    if (fee.lte(ONEAXC.mul(new BN(10))) || fee.lte(outputTotal)) {
      return true
    } else {
      return false
    }
  ...
  buildBaseTx = async (
    ...
    const feeAssetID: Buffer = await this.getAXCAssetID()
